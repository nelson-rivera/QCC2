...
if($option=="add"){
    ...
    $query=$connection->prepare(sql_save_cargo_contacto());
...
if($option=="update"){
    ...
    try {
        $connection->beginTransaction();
        $query=$connection->prepare(sql_update_contacto_cargo());
        $query->bindParam(':idcontacto_cargo', $idpcon);
        $query->bindParam(':cargo', $cargo);
...
if($option=="delete"){
    ...
    try {
        $connection->beginTransaction();
        $query=$connection->prepare(sql_delete_contacto_cargo());
        $query->bindParam(':idcontacto_cargo', $pcon);
        $query->execute();
        $connection->commit();
        $response['status']=1;
